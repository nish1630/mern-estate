  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          setIamgeUploadError("Image upload failed (2 mb max per image)");
          setUploading(false);
        });
    } else {
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="p-3 text-green-700 border border-green-700 rounded uppercase hover:shadow disabled:opacity-80"
            >
          ...
          <button className="p-3 bg-slate-700 text-white rounded-lg uppercase opacity-95 disabled:backdrop-opacity-80">
            Create Listing
          </button>
        </div>
      </form>
